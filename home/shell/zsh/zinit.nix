{
  ...
  zinitPluginStr = plugin: let
    ...
    target =
      if isLocal
      then localPath
      else if plugin.snippet != null
      then plugin.snippet
      else plugin.name;

    loadType =
      if plugin.snippet != null
      then "snippet"
      else "light";

    nixIce =
      if isLocal
      then " id-as'${plugin.name}' run-atpull"
      else "";
    finalIce =
      if plugin.ice != "" || nixIce != ""
